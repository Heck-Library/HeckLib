...
export function RandomPath(objects: BaseObject[], pointDefs: Map<string, AnyAnimation>) {
    if (!used) {
        used = true;
    ...
    objects.forEach((obj) => {
        ...
        obj.CustomData.Animation.OffsetPosition = `GrpOP${Random(1, 32)}`;
        obj.CustomData.Animation.OffsetWorldRotation = `GrpOWR${Random(1, 32)}`;
        obj.CustomData.Animation.LocalRotation = `GrpLR${Random(1, 32)}`;
        obj.CustomData.Animation.Dissolve = "GrpD";
    });
